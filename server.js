...
// ── Sesiones pendientes ───────────────────────────────────────────────────────
// Estructura: { pendingAction, pendingQuery, pendingEmailId, pendingContent,
//               pendingIntentAction, account, lastEmailsContext }
...
function detectEmailIntent(text = '') {
  ...
  if (/\belimina\b|\bborra\b|\bquita\b|\bsuprime\b|\bborrar\b|\beliminar\b|\bpapelera\b|\bpurga\b/.test(t)) return { action: 'eliminar', account };
  if (/\barchiva\b|\barchivar\b/.test(t)) return { action: 'archivar', account };
  if (/env[íi]a|enviar|manda|mandar.*email|escribe.*email|redacta|componer|crear.*email|nuevo.*email/.test(t)) return { action: 'redactar', content: text, account };
  if (/responde|responder|contesta|contestar|reply/.test(t)) return { action: 'responder', account };
  if (/prioriza|priorizar|importante|urgente|organiza.*correo|organizar.*email|orden.*importancia/.test(t)) return { action: 'priorizar', account };
  if (/busca|buscar|encuentra|encontrar|email.*de|correo.*de|email.*sobre|correo.*sobre|de parte de/.test(t)) return { action: 'buscar', query: text, account };
  ...
  if (/lee|leer|revisar|revisa|mira|muestra|dame|dime|cu[aá]ntos|tengo.*email|tengo.*correo|correo.*nuevo|email.*nuevo|no le[íi]dos|[uú]ltimo.*correo|[uú]ltimo.*email|recientes|nuevos/.test(t)
      || (esEmail && /qu[eé]|cu[aá]l|hay|tengo|ver|revisa|dame|dime|mira|muestra/.test(t))) return { action: 'leer', account };
  if (/marca.*le[íi]do|marcar.*le[íi]do|marca.*no le[íi]do/.test(t)) return { action: 'marcar', markAs: /no le[íi]do/.test(t) ? 'noleido' : 'leido', account };
  if (esEmail || isOng || isTrabajo) return { action: 'leer', account };
  if (/personal|mi gmail|mi bandeja/.test(t)) return { action: 'leer', account: 'personal' };
  return null;
}
...
function detectCreatorIntent(text = '') {
  const t = text.toLowerCase();
  if (/crea un flujo|crear un flujo|crea una automatizaci[oó]n|automatiza|quiero que cuando|cada vez que.*haz|programa un flujo/.test(t)) return { descripcion: text };
  return null;
}
...
// ── Extraer primer emailId de resultados n8n ──────────────────────────────────
function extractFirstEmailId(n8nResult) {
  try {
    const str = JSON.stringify(n8nResult);
    const match = str.match(/ID[:\s]+([a-f0-9]{10,})/i) ||
                  str.match(/"id"\s*:\s*"([a-f0-9]{10,})"/i);
    return match ? match[1] : '';
  } catch (e) { return ''; }
}

// ── Extraer email del remitente del primer resultado ──────────────────────────
function extractFirstFromEmail(n8nResult) {
  try {
    const str = JSON.stringify(n8nResult);
    const match = str.match(/De:\s*[^<\n]*<([^>]+)>/i) ||
                  str.match(/"from"\s*:\s*"[^"]*<([^>]+)>"/i) ||
                  str.match(/"From"\s*:\s*"[^"]*<([^>]+)>"/i);
    return match ? match[1] : '';
  } catch (e) { return ''; }
}

// ── Extraer contenido que quiere enviar el usuario ────────────────────────────
function extractUserContent(text = '') {
  const match = text.match(/dic[ié]ndole[:\s]+(.+)/i) ||
                text.match(/diciend[oa][:\s]+(.+)/i) ||
                text.match(/que diga[:\s]+(.+)/i) ||
                text.match(/mensaje[:\s]+(.+)/i) ||
                text.match(/responde[^:]*:[:\s]+(.+)/i);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/api/chat') {
      ...
      const pendingContent      = serverSession.pendingContent      || null;
      const pendingIntentAction = serverSession.pendingIntentAction || null;
      ...
      console.log('  pendingIntentAction:', pendingIntentAction);
      console.log('  pendingContent:', pendingContent);
      console.log('  pendingEmailId:', pendingEmailId);
      console.log('  emailIntent:', emailIntent ? emailIntent.action : null);
      console.log('  serverSession keys:', Object.keys(serverSession));

      // ── Gmail / N8N ───────────────────────────────────────────────────────
      if (hasGmailUrls && (emailIntent || (isConfirmation && (pendingAction || pendingIntentAction)))) {
        ...
        // ── CASO 1: Confirmación de acción normal (eliminar lote, etc) ────
        if (isConfirmation && pendingAction) {
          ...
          console.log('\n═══ CONFIRMACIÓN NORMAL a n8n ═══');
          console.log('  payload:', JSON.stringify(n8nPayload));

        // ── CASO 2: Confirmación tras buscar_para_confirmar ───────────────
        } else if (isConfirmation && pendingIntentAction) {
          // El usuario confirmó el email encontrado → ejecutar la acción real
          n8nPayload = {
            text: pendingContent
              ? `${pendingIntentAction} al email ${pendingEmailId} diciéndole: ${pendingContent}`
              : sanitize(lastMsg),
            confirmed: false,
            emailId: pendingEmailId || '',
            query: pendingQuery || '',
          ...
          console.log('\n═══ ACCIÓN TRAS CONFIRMAR BÚSQUEDA a n8n ═══');
          console.log('  payload:', JSON.stringify(n8nPayload));

        // ── CASO 3: Petición nueva ────────────────────────────────────────
        } else {
          if ((pendingAction || pendingIntentAction) && emailIntent) {
            pendingSessions.delete(sessionId);
            console.log('  🗑️ Sesión pendiente limpiada por nueva petición');
          }
          n8nPayload = {
            text: sanitize(lastMsg),
            autoSend: false,
        ...
        try {
          ...
          console.log('\n═══ RESPUESTA de n8n ═══');
          console.log('  resultado:', n8nResultStr.substring(0, 600));
          console.log('  needsConfirmation:', n8nResult.needsConfirmation);
          console.log('  action devuelta:', n8nResult.action);

          if (n8nResult) {
            const hasPending = n8nResult.needsConfirmation === true;

            // ── Detectar si fue búsqueda para confirmar acción posterior ──
            const wasSearchForConfirm = (
              !isConfirmation &&
              n8nResult.action === 'buscar' &&
              (emailIntent?.action === 'responder' || emailIntent?.action === 'eliminar')
            );

            if (hasPending) {
              // Confirmación normal (eliminar, etc)
              pendingSessions.set(sessionId, {
                ...
                lastEmailsContext: serverSession.lastEmailsContext || null,
                account
              });
              setTimeout(() => pendingSessions.delete(sessionId), 10 * 60 * 1000);
              console.log('  ✅ Sesión pendiente normal guardada');

            } else if (wasSearchForConfirm) {
              // Guardar intención + emailId + contenido para ejecutar al confirmar
              const firstEmailId = extractFirstEmailId(n8nResult);
              const firstFromEmail = extractFirstFromEmail(n8nResult);
              const extractedContent = extractUserContent(lastMsg);

              pendingSessions.set(sessionId, {
                pendingIntentAction: emailIntent.action, // 'responder' o 'eliminar'
                pendingEmailId: firstEmailId,
                pendingQuery: firstFromEmail ? `from:${firstFromEmail}` : '',
                pendingContent: extractedContent,
                lastEmailsContext: n8nResultStr.substring(0, 3000),
                account
              });
              setTimeout(() => pendingSessions.delete(sessionId), 10 * 60 * 1000);
              console.log('  ✅ Sesión buscar_para_confirmar guardada — emailId:', firstEmailId, 'content:', extractedContent);

            } else if (isConfirmation) {
              // Limpiar pendiente pero conservar contexto
              const prevCtx = serverSession.lastEmailsContext || null;
              pendingSessions.delete(sessionId);
              if (prevCtx) {
                pendingSessions.set(sessionId, { lastEmailsContext: prevCtx, account });
            ...
            // Instrucción especial cuando esperamos confirmación del usuario
            const currentSessNow = pendingSessions.get(sessionId) || {};
            const waitingConfirm = !!currentSessNow.pendingIntentAction;
            const confirmInstr = waitingConfirm
              ? `\nIMPORTANTE: Se han encontrado emails. Muestra el más reciente (De, Asunto, Fecha, Extracto breve) y pregunta: "¿Es este el correo al que quieres ${currentSessNow.pendingIntentAction === 'responder' ? 'responder' : 'eliminar'}? Responde sí para confirmar."`
              : '';
...
- Si needsConfirmation=true → muestra el preview y pide confirmación: "¿Confirmas? Responde sí"
...
${confirmInstr}`,
            ...
            console.log('  → hasPending=%s waitingConfirm=%s', hasPending, waitingConfirm);
      ...
      // ── Contexto previo sin intención nueva ───────────────────────────────
...
Eres un asistente de email. El usuario se refiere a emails ya mostrados.
Usa el contexto para entender a qué email se refiere y ayudarle.`,
            messages: [
